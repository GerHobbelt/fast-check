import { anyDomain, domain } from '../../../../src/check/arbitrary/HostArbitrary';
...
const isValidDomain = (t: string) => {
  ...
  return t.split('.').every(sd => rfc1123SubDomain.test(sd)) && t.length <= 63;
};
...
describe('DomainArbitrary', () => {
  describe('anyDomain', () => {
    genericHelper.isValidArbitrary(() => anyDomain(), {
      isValidValue: (g: string) => isValidDomain(g)
    });
  });
  describe('domain', () => {
